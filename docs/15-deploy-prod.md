...
| Автодеплой      | задача «Multa Autodeploy», 5 мин   | `tools/autodeploy.ps1`: пуш в `main` доезжает сам; лог → `autodeploy.log`     |
...
### Автодеплой (задача «Multa Autodeploy»)

С 22.08.2026 деплой не требует человека: задача планировщика раз в 5 минут (под `vasil`, как остальные задачи проекта) запускает `tools/autodeploy.ps1`. Пуш в `main` доезжает до прода сам за ≤5 минут.

Что делает: `git fetch` → сравнивает `origin/main` с коммитом, из которого **собран стек** (`C:\projects\multa\.deployed`) → при расхождении `checkout -f`, бэкап базы в `C:\backups\multa-autodeploy` (5 последних), `deploy.cmd`, смоук `/v1/health`, запись коммита как задеплоенного.

Сравнение идёт именно с `.deployed`, а не с рабочим деревом: дерево может уже держать новый код, пока контейнеры старые — ровно то состояние, в котором прод остаётся, если ручной деплой пропустили.

Чего задача не делает намеренно:

- **не запускает второй деплой поверх идущего** — лок `.autodeploy.lock` с истечением по времени (сборка идёт минуты, задача просыпается чаще; лок старше 40 минут считается брошенным);
- **не пересобирает сломанный коммит по кругу** — коммит помечается задеплоенным, как только сборка завершилась, даже если смоук не ответил: битое исправляется следующим коммитом;
- **не деплоит без бэкапа** — миграции идут при старте api, и к моменту, когда что-то выглядит не так, схема уже уехала.

Проверить: `ssh muspelheim 'powershell -NoProfile -Command "Get-Content C:\projects\multa\autodeploy.log -Tail 5"'`. Лог пишется только когда что-то произошло — тишина означает «нового коммита не было».

Ручной деплой остался и работает как раньше (`deploy.cmd`): он нужен, когда надо собрать не то, что в `main`, или когда задача выключена.

**Грабля, на которой скрипт умер в первом прогоне:** git пишет «From github.com:…» в stderr даже при успехе, а PowerShell с `ErrorActionPreference = 'Stop'` превращает это в фатальный `NativeCommandError` — скрипт падал на УДАЧНОМ fetch, до первой строки лога. Поэтому git и `pg_dump` вызываются через `cmd`, а судим по коду выхода.
