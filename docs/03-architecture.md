...
2. `getRate(base, quote, date, workspaceId?)`: fx_rates → ленивая дотяжка из API (исторические курсы вечны — кешируем навсегда) → выходные: последний рабочий день → нет данных: просим ручной ввод.
   **Личный курс важнее котировки на ту же дату.** Курс дня выплаты человек вводит руками, глядя на табло обменника (issue #48); он живёт в `fx_manual_rates` со скоупом воркспейса и подмешивается к котировкам только когда `getRate` знает воркспейс. Писать его в `fx_rates` нельзя: та таблица общая для всех, и личный факт протекал бы в чужие планы (правило 7). Свежесть при этом важнее источника — вчерашний ручной курс не побеждает сегодняшнюю котировку.
