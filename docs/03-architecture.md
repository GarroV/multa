...
| Интеграционные api | `apps/api/test/*.test.ts` | роуты целиком: изоляция workspace, каскад и сжатие, факт, исполнение, пересборка, чеки, размен, прогноз |
...
CI (`.github/workflows/ci.yml`) поднимает `postgres:16-alpine` сервисом, гоняет `pnpm typecheck` + `pnpm test` на пустой базе с нуля, затем собирает прод-web и прогоняет смоук бандла.

## Инвентарь API (v1)

> Хэнд-мейд, сверяется с `apps/api/src/app.ts` и `apps/api/src/routes/*`. Все ручки, кроме `/v1/health`
> и `/v1/auth/*`, требуют сессии; скоуп workspace берётся из токена, клиент его не передаёт (правило 7).
