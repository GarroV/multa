...
Это тот же набор и тот же порядок, что были в GitHub Actions. **Он гоняется автоматически** —
`pre-push` хук из `.githooks/` вызывает `pnpm verify` и не пускает пуш, если что-то красное. Хук
включается сам при `pnpm install` (скрипт `prepare` ставит `core.hooksPath`), поэтому он переживает
клон и работает у всех, а не только у того, кто настроил его руками.
