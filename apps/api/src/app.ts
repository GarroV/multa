...
app.get('/v1/me', requireAuth, async (c) => {
  ...
  return c.json({
    ...
    role: ws ? role : null,
    /*
     * «Сегодня» в таймзоне воркспейса (#109). Клиент вычислял дату сам через `toISOString()` — это
     * UTC, и между местной полуночью и смещением зоны они расходились на день: трата, введённая в
     * 00:48 в Белграде, уезжала во вчерашний день, а на стыке полумесяцев — в предыдущий период.
     * Правильную дату знает только сервер, у него таймзона воркспейса.
     */
    today: ws ? today(ws.timezone) : new Date().toISOString().slice(0, 10),
  });
});
