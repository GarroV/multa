...
/**
 * Код валюты по ISO 4217: три латинские буквы.
 *
 * Объявлен здесь, до первой схемы, которая его использует. Раньше базовая валюта воркспейса имела
 * свою проверку (`z.string().length(3)`), не связанную с общей, — и мимо неё на прод прошло «СОМ»
 * кириллицей вместо KGS (найдено 19.08.2026 у живого пользователя). Два правила для одного и того
 * же понятия расходятся ровно так: одно починили, второе осталось.
 *
 * Почему это ломает всё тихо: курса для «СОМ» нет ни в одном источнике, поэтому каждая строка плана
 * уходит в «нерешённые», а `exponentOf` отдаёт неизвестному коду дефолтные два знака — суммы
 * выглядят правдоподобно. Человек видит пустой план и не понимает, почему.
 *
 * Регистр приводим сами: клиенты присылают и «rub», и «RUB», подмена регистра безопасна — в отличие
 * от подмены алфавита.
 */
const ccy = z
  .string()
  .regex(/^[A-Za-z]{3}$/, 'код валюты — три латинские буквы (ISO 4217), например RSD')
...
/**
 * Валюты, между которыми живёт этот воркспейс (решение владельца 06.08.2026).
 *
 * Список настраиваемый, а не зашитый: у каждого он свой — рубли с тенге у одного, евро с сомами у
 * другого. Дефолт покрывает случаи, ради которых продукт и делается, а полный справочник ISO сюда
 * не годится: в выпадашке из ста семидесяти позиций нужную ищут дольше, чем набирают руками.
 *
...
export const planGridQuerySchema = z.object({
  /*
   * Потолок 24, как у аналитики: длина периода у всех разная, и «12 периодов» это полгода при
   * выплатах дважды в месяц, но год при ежемесячных. Планировать отпуск или закрытие долга на три
   * месяца вперёд нельзя, а именно столько давал прежний потолок в 12 при шаге в полмесяца
   * (вопрос владельца 16.08.2026). По умолчанию 12 — год для ежемесячных, полгода для полумесячных.
   */
...
/**
 * Список трат для экрана истории (issue #137).
 *
 * Предел выдачи обязателен: у владельца четыре года импортированной истории, и без него экран тянул
 * бы тысячи строк одним ответом при каждом открытии. Верхняя граница — не бюрократия, а тот же
 * довод: страница длиннее пятисот строк уже не читается, а грузится заметно.
 */
export const transactionListSchema = z.object({
  from: isoDate.optional(),
  to: isoDate.optional(),
  /** Фильтр по категории. Принадлежность проверяет хендлер: id приходит от клиента (правило 7). */
  categoryId: z.string().uuid('categoryId — uuid категории').optional(),
