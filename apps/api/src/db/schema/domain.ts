...
export const debts = pgTable(
  'debts',
  {
    ...
    /**
     * Разбивка платежа по источникам дохода: сколько уходит с каждой выплаты (запрос владельца
     * 16.08.2026: «плачу и с аванса и с зарплаты, разные суммы»).
     *
     * Отдельно от `amountSteps`, а не вместо: ступени отвечают «сколько с такого-то момента», а
     * разбивка — «сколько с какой выплаты». Пустой список означает прежнее поведение, поэтому все
     * заведённые долги продолжают считаться как раньше.
     */
    paymentsBySource: jsonb('payments_by_source'),
    /**
     * Окно платежей: «платим с ноября по февраль» (issue #117). Границы включительные.
     *
     * Отдельно от `dueDate`: тот отвечает «к какому сроку закрыть» и служит для расчёта платежа, а
     * окно — «в какие периоды деньги вообще уходят». Пусто — платим всегда, как было раньше.
     */
    paysFrom: date('pays_from'),
    paysUntil: date('pays_until'),
    /**
