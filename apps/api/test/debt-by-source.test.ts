...
test('вне окна платежей долг не берёт денег ни в плане, ни в таблице', async () => {
  ...
  /*
   * Ноль, а не отсутствие (issue #134, приведено к одному правилу 19.08.2026). Раньше тест ждал
   * `undefined`: вне окна долг исчезал с «Плана», но в таблице ниже показывался нулём — два экрана
   * про одни и те же деньги давали разный набор строк. Смысл проверки тот же и остаётся в силе:
   * денег вне окна долг не берёт. Но строку человек видит — иначе выглядит, будто долг удалён.
   */
  // `debtIn` отдаёт plannedMinor строки долга: `undefined` означало бы, что строки нет вовсе.
  expect(debtIn(await getPlan(client)), 'долг вне окна обязан быть виден строкой').toBe('0');
