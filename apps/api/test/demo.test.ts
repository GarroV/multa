...
describe('вход в демо', () => {
  test('без регистрации выдаётся сессия и наполненный воркспейс', async () => {
    ...
    /*
     * Порог, а не «больше нуля» — но честный ноль это не то же самое, что маленькая сумма.
     * «Health» и «General» в демо-сиде намеренно без бюджета (список `BUDGETS` их не содержит), и
     * с 19.08.2026 такая категория показывается в allocations нулём, а не пропадает из списка
     * вовсе (жалоба владельца «продукты не добавляются» — категория без бюджета исчезала из
     * каскада целиком). Порог ловит другое: строку на 30 RUB вместо 3 000 — так уже случилось с
     * fixed-конвертом, где ruleValue хранился в minor units, а форма правки подставляла его как
     * major. Нулевые строки этот класс ошибки дать не могут, поэтому пропускаем именно их.
     */
    for (const a of plan.allocations) {
      if (a.allocatedMinor === '0') continue;
