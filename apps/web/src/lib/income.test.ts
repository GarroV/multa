...
describe('nextPeriodStart', () => {
  /*
   * Умолчание «новый долг платится со следующей выплаты» (issue #121, замечание владельца: «я внёс
   * новый долг, и он должен привязаться к следующему периоду, не к текущему»). Функция была, а
   * теста на неё — нет, хотя от неё зависит, не откусит ли новое обязательство от уже прожитого
   * периода задним числом.
   */
  const rhythm = { kind: 'monthly-days' as const, days: [10, 25] };

  it('даёт начало периода, который ещё не начался', () => {
    // 20-е: текущий период начался 10-го, следующий начнётся 25-го.
    expect(nextPeriodStart(rhythm, 'as-is', '2026-08-20')).toBe('2026-08-25');
  });

  it('в день выплаты уводит на следующую, а не оставляет в сегодняшней', () => {
    /*
     * Строгое «позже сегодня», а не «сегодня или позже»: период, начавшийся сегодня, уже идёт, и
     * долг, привязанный к его началу, платился бы прямо сейчас — ровно то, от чего уходили.
     */
    expect(nextPeriodStart(rhythm, 'as-is', '2026-08-25')).toBe('2026-09-10');
  });

  it('ритм без выходных-правил и с переносом даёт одну и ту же логику', () => {
    expect(nextPeriodStart(rhythm, 'before', '2026-08-20')).toBe('2026-08-25');
  });

  it('на негодном ритме возвращает null, а не выдуманную дату', () => {
    // Форма не должна подставлять дату, вычисленную из мусора: пусть поле останется пустым.
    expect(nextPeriodStart({ kind: 'nonsense' }, 'as-is', '2026-08-20')).toBeNull();
    expect(nextPeriodStart(null, 'as-is', '2026-08-20')).toBeNull();
  });
});
