 ...
 * локали и даём переписать. Живут здесь, а не в компоненте, потому что важен не текст, а привязка к
 * числам: аванс закрывает первую половину текущего месяца и приходит ближе к её концу, окончательный
 * расчёт за прошлый месяц — в начале следующего (ТК РФ ст. 136, не позднее 15 числа). Значит 10 —
 * зарплата, 25 — аванс. Обратный порядок владелец увидел на своём экране 11.08.2026.
 */
const SEED_LABELS: Record<'ru' | 'en', readonly [string, string]> = {
  ru: ['Зарплата', 'Аванс'],
  en: ['Salary', 'Advance'],
};

export function seedPayouts(locale: 'ru' | 'en'): PayoutForm[] {
  const [tenth, twentyFifth] = SEED_LABELS[locale];
  // Суммы пустые намеренно: подставленная цифра выглядит как ответ, которого мы не знаем.
  return [
