...
/*
 * «Оплатить» отмечает исполнение обязательства (жалоба владельца 16.08.2026: «оплатить — ничего не
 * делает»).
 *
 * Ручка подтверждения на сервере отвечает 200 и меняет статус на `confirmed` — проверено прямым
 * запросом. Значит вопрос к экрану: нажатие обязано менять то, что человек видит, иначе он жмёт
 * второй и третий раз, не понимая, сработало ли.
 */
test('«оплатить» отмечает обязательство исполненным прямо на экране', async ({ page }) => {
  await page.goto('/plan');
  const row = page.locator('.prow', { hasText: /Ozon|Bank credit|Рассрочка|Кредит/ }).first();
  await row.waitFor();

  await row.getByRole('button', { name: /^оплатить$|^pay$/i }).click();

  /*
   * Ищем по НОВОЙ подписи, а не по старой: после успеха кнопка называется «внесено», и локатор по
