...
test('мастер-режим показывает год вперёд и не уезжает вбок', async ({ page }) => {
  /*
  ...
  /*
   * Двенадцать колонок периодов + колонка названий. Было шесть, но при выплатах дважды в месяц это
   * ровно три месяца — планировать отпуск или закрытие долга на таком горизонте нельзя (вопрос
   * владельца 16.08.2026). Горизонт переключается прямо в подвале таблицы.
   */
  await expect(grid.locator('.mgrid-row-periods .mgrid-cell')).toHaveCount(12);
