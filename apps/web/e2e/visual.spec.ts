...
/*
 * Лист ввода не растягивает элементы по высоте ячейки (замечание владельца 16.08.2026).
 *
 * Лист — двухколоночная сетка, и по умолчанию ячейка растягивает содержимое на всю высоту ряда.
 * Кнопка «Записать» оказывалась ростом 415px — во всю высоту соседней плитки со списком трат, — и
 * лист читался как набор случайно раскиданных блоков.
 *
 * Проверяем кнопку: у неё есть собственный разумный рост, и он не зависит от того, что стоит рядом.
 */
test('кнопки листа ввода не растягиваются во всю высоту ряда', async ({ page }) => {
  await resetDemo(page);
  await enterDemo(page);
  await page.goto('/plan');
  ...
  // Именно кнопка отправки, а не первая попавшаяся: крестик закрытия тоже button.btn.
  const submit = sheet.getByRole('button', { name: /^Записать$|^Log it$/ });
