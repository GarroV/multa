...
export const ru: Record<TranslationKey, string> = {
  ...
  // Регулярный платёж правится разово: «в этом месяце счёт другой» (issue #154).
  'plan.master.editOnce': 'Только в этом периоде — нажми, чтобы поправить',
  'plan.master.editFailed': 'Правка не сохранилась. Попробуй ещё раз.',
  'plan.master.editNoRate': 'Правка не сохранилась: нет курса на сегодня для этой валюты.',
  'plan.master.editReadonly': 'Эту строку через таблицу не поправить — доход правится на «Плане».',
  'plan.master.editPast': 'Прошлый период не правится: закрытый план — это история.',
  ...
  // Прочерк вместо «0 / 0» у категории без бюджета (issue #139): подсказка объясняет прочерк.
  'plan.category.noBudget': 'бюджет не задан',
  ...
  'obl.name': 'Название',
  'obl.window': 'Платим с… по…',
  'obl.window.from': 'Платим с',
  'obl.window.until': 'Платим по',
  'obl.window.hint':
    'В какие периоды долг вообще берёт деньги. Обе границы включительно; пустое поле — без ограничения с этой стороны.',
  'obl.startsNext': 'Платить начнём с {date} — со следующей выплаты',
  'obl.settings': 'Настройки долга',
  ...
  // Настройки строки прямо из таблицы (issue #154, запрос владельца 22.08.2026).
  'grid.row.settings': 'Настройки строки',
  'grid.row.gone': 'Строки больше нет — её удалили или переименовали в другом окне.',
  'grid.row.categoryBudget': 'Бюджет категории задаётся в ячейке периода: у каждого периода свой.',
  'grid.row.currencyFixed':
    'Валюта строки — {ccy}. Сменить её здесь нельзя: суммы записаны в этой валюте, и подмена молча переозначила бы их. Нужна другая валюта — заведи строку заново.',
  'grid.row.protect': 'Защищена от автоматического сжатия',
  'obl.perPeriod': 'Взнос за период',
  'obl.payment': 'Платёж',
  ...
  // Настройки сгруппированы по поверхностям (22.08.2026): что про дашборд, что про таблицу.
  'set.behaviour': 'ДАШБОРД: ПЛАН И ТЕМП',
  ...
  // Настройки мастер-таблицы: жили в коде и в localStorage, теперь настройка (22.08.2026).
  'set.grid.title': 'МАСТЕР-ТАБЛИЦА',
  'set.grid.horizon': 'Горизонт, периодов',
  'set.grid.horizon.hint':
    'Сколько периодов выплат таблица показывает за раз. Считаем в периодах, а не в месяцах: при выплатах дважды в месяц 12 периодов — это полгода, при ежемесячных — год.',
  'set.grid.nameWidth': 'Ширина первого столбца, px',
  'set.grid.nameWidth.hint':
    'Столбец с названиями строк. Ширину можно тянуть мышью в самой таблице — здесь она числом, чтобы поставить ровное значение. От 140 до 560: уже — не читаются заголовки разделов, шире — колонки сумм уезжают за экран.',
  'set.grid.sections': 'Показывать разделы',
  'set.grid.sections.hint':
    'Тумблер убирает раздел с экрана, но не удаляет данные: строки, история и ручки API остаются на месте, и раздел можно вернуть в любой момент. Цели и валютные корзины скрыты по умолчанию — это та же механика, что накопления и долги, только под другими именами.',
  'set.signals': 'ДАШБОРД: КОГДА ПРЕДУПРЕЖДАТЬ',
  ...
  'imp.skip.no_amount': 'нет суммы — строки',
  'imp.skip.no_date': 'нет даты — строки',
  'imp.skip.no_category': 'нет категории — строки',
  'imp.skip.more': ' и ещё {rest}',
