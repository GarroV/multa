...
/*
 * Прогноз не должен подменять переданные платежи плоской суммой.
 *
 * Найдено сверкой кода 17.08.2026 после того, как долгам добавили разбивку по выплатам и окно
 * «платим с… по…». Вызывающий передаёт платёж НА КАЖДЫЙ период — это единственная правда; плоское
 * `paymentMinor` осталось для старых данных. Но в двух местах ядро всё ещё бралось за плоское
 * число, и обе ошибки врали в приятную сторону.
 */
describe('прогноз опирается на переданные платежи, а не на плоскую сумму', () => {
  it('периоды за пределами переданного массива не платят, а не берут плоскую сумму', () => {
    /*
     * Массив короче горизонта — значит про дальние периоды вызывающий ничего не сказал. Подставлять
     * туда плоский платёж значило бы обещать закрытие долга на данных, которых нам не давали.
     */
    const events = forecastTimeline({
      asOf,
      periodsAhead: 6,
      ...
      debts: [
        {
          ...
          remainingMinor: 9000000n,
          paymentMinor: 4500000n,
          // Про первые два периода известно: платим по копейке. Дальше — молчание.
          paymentsByPeriod: [100n, 100n],
        },
      ],
      goals: [],
    });
    expect(events.find((e) => e.kind === 'debt_closed')).toBeUndefined();
  });

  it('«освободится» показывает платёж закрывающего периода, а не плоскую сумму', () => {
    /*
     * Долг гасится по 1 000 за период, а плоское поле хранит 20 000 (например, до появления
     * разбивки). Сказать «освободится 20 000 за период» значило бы обещать вчетверо больше денег,
     * чем в действительности освободится.
     */
    const events = forecastTimeline({
      asOf,
      periodsAhead: 6,
      ...
      debts: [
        {
          ...
          name: 'Кредитка',
          currency: 'RUB',
          remainingMinor: 300000n,
          paymentMinor: 2000000n,
          paymentsByPeriod: [100000n, 100000n, 100000n],
        },
      ],
      goals: [],
    });
    const freed = events.find((e) => e.kind === 'freed_money');
    expect(freed?.periodsAway).toBe(3);
    expect(freed?.amountMinor).toBe(100000n);
  });
});
