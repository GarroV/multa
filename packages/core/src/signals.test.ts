...
describe('burnRate — темп трат и когда кончатся деньги (Спринт 4)', () => {
  it('считает дневной темп по прошедшим дням, а не по всему периоду', () => {
    ...
    const r = burnRate({ livingMinor: 100000n, spentLivingMinor: 10000n, period, asOf: '2026-07-29' });
  ...
  it('темп выше плана — говорит, какого числа кончатся деньги', () => {
    ...
    const r = burnRate({ livingMinor: 100000n, spentLivingMinor: 60000n, period, asOf: '2026-07-29' });
  ...
  it('деньги уже кончились — дата в прошлом не выдумывается, берётся сегодня', () => {
    const r = burnRate({ livingMinor: 100000n, spentLivingMinor: 120000n, period, asOf: '2026-07-29' });
  ...
  it('первый день периода: делим на один день, а не на ноль', () => {
    const r = burnRate({ livingMinor: 100000n, spentLivingMinor: 3000n, period, asOf: '2026-07-25' });
  ...
  it('последний день периода — прогноз не нужен, до выплаты дошли', () => {
    const r = burnRate({ livingMinor: 100000n, spentLivingMinor: 90000n, period, asOf: '2026-08-10' });
