 ...
 * (решение основателя; 01-domain-model §Исполнение). Внутри уровня категорий —
 * сначала обычные, защищённые не режутся автоматически.
 *
...
function compressionTiers(plan: PlanItem[]): number[][] {
  const idxOf = (pred: (p: PlanItem) => boolean): number[] =>
    plan.map((p, i) => (pred(p) ? i : -1)).filter((i) => i >= 0);
  return [
    idxOf((p) => p.targetKind === 'goal'),
    idxOf((p) => p.targetKind === 'envelope'),
    idxOf((p) => p.targetKind === 'category' && !p.protected),
    // debt, bucket, protected category — не режутся автоматически
  ];
}

export function cascade(incomeMinor: bigint, plan: PlanItem[]): CascadeResult {
  const allocated = plan.map((p) => p.plannedMinor);
  const totalPlanned = sum(allocated);

  let deficit = totalPlanned - incomeMinor;
  if (deficit > 0n) {
    for (const tier of compressionTiers(plan)) {
      if (deficit <= 0n) break;
