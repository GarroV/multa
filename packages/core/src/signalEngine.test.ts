...
describe('buildSignals', () => {
  ...
  it('категория стабильно дороже плана — предложение с конкретной суммой', () => {
    ...
    /*
     * Второе действие — «взять недостающее из другой строки» — обязано нести ту же сумму (#106).
     * Без неё веб открывал модал пересборки с нулём, запрос вариантов не уходил вовсе, и человек
     * видел пустое окно: сердце продукта выглядело сломанным ровно в момент, когда оно нужно.
     */
    expect(signal.actions).toContainEqual({
      kind: 'rebalance',
      targetId: 'c1',
      needMinor: 13_350_00n - 9_000_00n,
    });
  });
