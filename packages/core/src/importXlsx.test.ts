...
describe('parseMasterGrid', () => {
  ...
  it('периоды читаются как даты, а строки — как статьи с суммами', () => {
    /*
     * Ожидание изменено осознанно (17.08.2026). Заготовка сама объявляет вторую строку числами
     * месяца (10, 25, 10), но прежняя проверка ждала первых чисел — то есть закрепляла, что разбор
     * эту строку ИГНОРИРУЕТ. На настоящем файле владельца это давало полумесячный план, разложенный
     * по первым числам, и он разъезжался с ритмом выплат.
     */
    const result = parseMasterGrid(grid, { currency: 'RUB' });
    expect(result.periods).toEqual(['2024-10-10', '2024-11-25', '2024-12-10']);
...
/*
 * Полумесячная раскладка «Кетчуп в Химках» — как она устроена В ФАЙЛЕ, а не в удобной заготовке.
 *
 * Найдено прогоном настоящего файла владельца 17.08.2026. В шапке месяц стоит ТОЛЬКО над первой
 * колонкой пары (в Excel это объединённая ячейка), над второй — пусто; числа месяца живут во второй
 * строке: 10 и 25. Разбор брал колонки, где в первой строке есть дата, — то есть каждую вторую, — и
 * молча выбрасывал все выплаты 25-го числа. На файле владельца это половина дохода и половина
 * расходов, исчезающие без единого признака.
 *
 * Прежняя заготовка теста ставила дату в КАЖДУЮ колонку, поэтому тест зеленел на раскладке, которой
 * не бывает. Здесь она воспроизведена буквально.
 */
describe('parseMasterGrid на полумесячной раскладке из файла', () => {
  const grid = [
    // Месяц над первой колонкой пары, вторая пуста — объединённая ячейка Excel.
    ['Месяц', '45566.0', '', '45597.0', ''],
    ['46222', '10.0', '25.0', '10.0', '25.0'],
    ['Доход', '110417.0', '46980.0', '101246.0', '46000.0'],
    ['Сбер', '10000.0', '10000.0', '9065.0', '13000.0'],
  ];

  it('берёт ВСЕ колонки, включая выплаты 25-го числа', () => {
    const result = parseMasterGrid(grid, { currency: 'RUB' });
    expect(result.periods).toHaveLength(4);
    expect(result.income?.amountsMinor).toEqual([11_041_700n, 4_698_000n, 10_124_600n, 4_600_000n]);
  });

  it('дата периода складывается из месяца шапки и числа второй строки', () => {
    // Иначе полумесячный план ложится на первые числа месяцев и разъезжается с ритмом человека.
    const result = parseMasterGrid(grid, { currency: 'RUB' });
    expect(result.periods).toEqual(['2024-10-10', '2024-10-25', '2024-11-10', '2024-11-25']);
  });

  it('статьи тоже читаются по всем колонкам', () => {
    const result = parseMasterGrid(grid, { currency: 'RUB' });
    const sber = result.lines.find((l) => l.name === 'Сбер')!;
    expect(sber.amountsMinor).toEqual([1_000_000n, 1_000_000n, 906_500n, 1_300_000n]);
  });
});
