...
describe('parseSpendJournal', () => {
  ...
  it('пустая «Сумма» достраивается из цены и количества, а не теряет строку', () => {
    /*
     * В настоящем файле у части строк итог не проставлен: есть только цена (и иногда количество).
     * Терять такую трату нельзя — деньги потрачены. Пустое количество означает одну штуку.
     */
    const rows2 = [
      ['Дата', 'Категория', 'Продукт', 'Стоимость', 'Количество', 'Сумма', 'Комментарий'],
      ['44896.0', 'Хозтовары', 'Молоток', '180.0', '', '', ''],
      ['44896.0', 'Продукты', 'Яблоки', '90.0', '3', '', ''],
    ];
    const result = parseSpendJournal(rows2, { currency: 'RUB' });
    expect(result.rows.map((r) => r.amountMinor)).toEqual([18_000n, 27_000n]);
  });

  it('явный ноль в «Сумме» — осознанный ноль, строка пропускается', () => {
    // 0 в итоге при непустой цене встречается в файле: это пометка «взяли, но не платили».
    const rows2 = [
      ['Дата', 'Категория', 'Продукт', 'Стоимость', 'Количество', 'Сумма', 'Комментарий'],
      ['44896.0', 'Продукты', 'Творог', '0.4', '', '0', ''],
    ];
    const result = parseSpendJournal(rows2, { currency: 'RUB' });
    expect(result.rows).toHaveLength(0);
    expect(result.skipped[0]?.reason).toBe('no_amount');
  });

  it('число в колонке «Продукт» — это не название позиции', () => {
    /*
     * В настоящем файле у половины строк в «Продукте» лежит цена или количество: человек не
     * заполнял название. Считать «229.99» позицией нельзя — такая «позиция» ломает и словарь
     * автокатегоризации, и статистику по товарам (найдено приёмкой на реальных данных).
     */
    const rows2 = [
      ['Дата', 'Категория', 'Продукт', 'Стоимость', 'Количество', 'Сумма', 'Комментарий'],
      ['44896.0', 'Неизвестное', '229.99', '', '1.0', '229.99', ''],
      ['44896.0', 'Продукты', 'Помидоры', '139.99', '1.0', '139.99', ''],
    ];
    const result = parseSpendJournal(rows2, { currency: 'RUB' });
    expect(result.rows[0]?.item).toBeNull();
    expect(result.rows[0]?.amountMinor).toBe(22_999n);
    expect(result.rows[1]?.item).toBe('Помидоры');
  });
...
describe('parseCategoryDictionary', () => {
  ...
  it('числовые ячейки словаря не становятся позициями', () => {
    // В настоящем «Словаре» встречаются числа («129», «223»): это остатки расчётов, а не товары.
    const map = parseCategoryDictionary([
      ['Декор', 'Гигиена'],
      ['223', 'Зубная паста'],
      ['Растения и утварь', '129'],
    ]);
    expect(map.has('223')).toBe(false);
    expect(map.has('129')).toBe(false);
    expect(map.get('зубная паста')).toBe('Гигиена');
    expect(map.get('растения и утварь')).toBe('Декор');
  });
