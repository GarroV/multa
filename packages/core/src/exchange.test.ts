...
const official = { from: 'RUB', to: 'RSD', rate: '1.1500000000', source: 'cbr', date: '2026-07-30' };
...
describe('exchangeResult — фактический курс и спред (01-domain-model §ExchangeOperation)', () => {
  it('обмен точно по официальному курсу — спреда нет', () => {
    const r = exchangeResult({ fromMinor: 10000000n, fromCurrency: 'RUB', toMinor: 11500000n, toCurrency: 'RSD', official });
  ...
  it('меняльник дал меньше — считаем недополученное и процент', () => {
    ...
    const r = exchangeResult({ fromMinor: 10000000n, fromCurrency: 'RUB', toMinor: 11270000n, toCurrency: 'RSD', official });
  ...
  it('дали больше официального — спред отрицательный, это выгода, а не ошибка', () => {
    const r = exchangeResult({ fromMinor: 10000000n, fromCurrency: 'RUB', toMinor: 11730000n, toCurrency: 'RSD', official });
  ...
  it('курс считается по major-суммам, а не по minor: exponent валют разный', () => {
    ...
    const r = exchangeResult({ fromMinor: 100000n, fromCurrency: 'RUB', toMinor: 1400n, toCurrency: 'JPY', official: jpy });
  ...
  it('без официального курса спред не выдумываем', () => {
    const r = exchangeResult({ fromMinor: 10000000n, fromCurrency: 'RUB', toMinor: 11270000n, toCurrency: 'RSD', official: null });
  ...
  it('нулевая отдача не роняет расчёт делением на ноль', () => {
    const r = exchangeResult({ fromMinor: 0n, fromCurrency: 'RUB', toMinor: 11500000n, toCurrency: 'RSD', official });
